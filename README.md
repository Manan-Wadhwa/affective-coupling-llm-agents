...
| A probe-readable affect signal in the receiver responds to the sender's steering dose (5 of 6 emotions, both models); the receiver's other-speaker projection moves comparably, so this is not shown to be the receiver's own state | present slopes +48 to +212; other-speaker +59 to +119 | `b1f.*`, `b1c.*`, `rev.other_probe_rises` · 20, 22, 27 |
| The estimator instability is the n < d regime and is largely geometric: projected to 100 PCs the logistic direction reproduces at 0.90; held-out projections of the two estimators correlate 0.90–0.96 | — | `a2l.*` · paper §3 |
...
| B1e's design on the 8B (out-of-prereg replication): H3 under the rule; only sad replicates across models on both outcomes; the control is matched on average only and adds no discriminating power over the random control there | sad −36.8 [−57.1, −16.7] (36%); afraid unreplicated | `b1e8.*` · 30 |
