...
**Provenance gap, stated up front.** The driver was meant to stamp the pre-registration's sha and commit into the result (PREREG §8). It did not: `_provenance.config.prereg.found` is `false` because the document was never staged to the sandbox. The ordering "hypotheses before data" therefore rests on git commit timestamps against `_provenance.started_utc`, not on the file's own stamp.
...
- **Provenance**: the prereg-stamp gap and the IST/UTC date mismatch are stated at the top of the report; `code_sha` matches the driver on disk. The fix for next time (stage the pre-registration to the box) is in the operator notes.
