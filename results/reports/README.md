...
| 29 | B1c's design on Llama-3-8B-abliterated — transfer pattern replicates; rule returns instrument_failed | `rev3/b1c_alllayer_llama3-abl.json` | numbers VERIFIED · 'floating-point tie' REJECTED (real null of the upper window + strict ≥) · dose not comparable across models (manipulation half as strong, less specific) · C3 corrected (3/6 fail §4.4; text_keep collapses 5/6) · 'same two emotions' narrowed to significance pattern · prereg stamp empty in both B1c files |
| 30 | B1e's design on the 8B (out-of-prereg replication) — H3 under the rule | `rev3/b1e_footprint_llama3-abl.json` | numbers VERIFIED (two C2 ranges corrected) · C4 'footprint discharged' WITHDRAWN (matched on average only; isotropic-norm directions; B1c's random control reproduced the contrasts) · afraid UNREPLICATED (no block vs baseline; models swap strong emotion) · only sad replicates |
