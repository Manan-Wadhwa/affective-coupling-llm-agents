...
Supersedes `ROADMAP.md` and revs 1–2. Incorporates the 2026-08-31 sandbox runs, the
novelty review, the code audit of `coupling_e3_ablate.py`, and the independent claims
audit (`AUDIT.md`, eighteen findings).
...
`results/sandbox_pull/*.json`, or a cited paper. Where a document disagrees with a result
...
**0.2 Provenance triage.** Classify every number in `paper.html`, `README.md` and
`dossier.html`:
...
- *Table 1's 8B column* (0.67 / 0.37 / 0.63 / 0.21 / 0.05) → **(d)**, not (c). It is
  verbatim the `dossier.html` Llama-3.1-8B column, which the dossier presents as a molab
...
**0.4 Correction sweep.** README, `paper.html`, `dossier.html`, `NOVELTY_REVIEW.md`,
commit `e03c17e`, `SHARED_WORKSPACE_SPEC.md:12`. Anchored to `AUDIT.md`'s eighteen
...
From `coupling_e3_ablate.py`: A's line is generated once (L146). B's reply is generated
...
`coupling_e2.py`'s own docstring calls it *"the load-bearing result."* In
...
`coupling_e2_ci.py` excludes by docstring — *"drop the model-breaking alpha=2 regime"* —
...
**B4.** The scramble is broken by construction in `coupling_e5_actpass.py`,
`behavioral_induction.py`, and the agentic arm: each permutes vectors captured under a
*single* emotion, preserving the emotion component. The induction arm's ≈0 result is
arithmetic, and `paper.html` §10 cites it as corroboration.
...
*Rev 3, compiled 2026-09-01. Supersedes `ROADMAP.md` and revs 1–2; that document's honesty
guardrail carries forward unchanged, and §1 is the clearest evidence so far of why it is
needed. Companion documents: `AUDIT.md` (claims audit), `NOVELTY_REVIEW.md` (novelty and
literature).*
