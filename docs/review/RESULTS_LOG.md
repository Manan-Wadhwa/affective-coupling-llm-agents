...
`a2_margin_qwen36-27b-l43.json`, `-l54.json` and `a2_margin_llama3-abl-l21.json` (8B, same regime; cos(lam, dom) 0.91 → 0.43): training accuracy 1.000, log-loss ≤ 0.002, ≤ 25 iterations for n = 75…2000 per half under C_n, C = 0.5 and C = 50 alike; cos(lam, weak) ≥ 0.975; the weak-penalty direction is no more reproducible (0.52 vs 0.58); cos(lam, dom) falls 0.97 → 0.63 with n while dom's split-half rises 0.57 → 0.97. The follow-up's logistic directions are the penalty's tie-break among separating hyperplanes in the n ≪ d regime, not likelihood-driven estimates; the critics' "likelihood takes over" reading is not what happens (the delta critique withdraws the stronger 'max-margin limit' wording: one weak-C point, direction increasingly penalty-sensitive with n). Registry `a2m.*` (3); report 26. Script `src/rev3/a2_margin_diag.py` (selftest reproduces the follow-up's fits to 1e-9).

## 2026-09-05 08:31Z — B1d pre-registered rank-5 subspace ablation complete (box 5): instrument_failed
Verdict **instrument_failed** under PREREG_B1d §2: `sub_all` removes ≥ 80% of readable affect for 3 of 6 emotions and shifts the dose-0 readout (desperate 0.11 → 0.50); the label-permuted control is not inert for 4 of 6 and itself blocks transfer for afraid (−49.5), sad (−26.7), angry (−44.3) (unadjusted CIs; BH-FDR q 0.051 each) — fitted high-variance ablations (removed norm 6.5–6.9 vs 0.66 for the rank-1 direction) reduce B's dose-response without emotion information. Descriptive median `sub_all − perm_all` 0.137 [−0.027, 0.286]; rank 5 vs rank 1 split (afraid +29, angry −61). `none`/`emo_all` reproduce B1c bit for bit. Prereg stamped in the file (sha 784cd649). Registry `b1d.*` (7); report 27 + blind critique. Consequence: B1c's afraid/sad specificity needs a footprint-matched rank-1 control.

## 2026-09-05 11:19Z — B1e complete (box 5): verdict **H3**
Fitted label-free rank-1 control (permuted-label dom direction, cosine 0.004 to the emotion direction, footprint 0.85×): inert for every emotion; the emotion direction blocks afraid −52.5 [−68.9, −35.8] (25%) and sad −28.4 [−54.2, −7.0] (29%) against it, BH-FDR q 0.001 / 0.028. The top principal direction (5.8×) blocks angry by 61% and calm but not afraid/sad. Registry `b1e.*` (3); report 28. Under the rule B1c's afraid/sad blocking survives a fitted label-free control; the critique keeps the footprint caveat (control removes less than B1c's inert random direction; 0.85×–5.8× unsampled) and withdraws the overlap explanation of B1d.
...
`b1e_footprint_llama3-abl.json`: H3 under the rule; sad blocked 36% against the control (q 0.002, secondary agrees), afraid 14% (q 0.08, no block against baseline). The delta critique withdraws 'footprint discharged' (matched on average only, sad 0.86×; B1c's random direction reproduced the contrasts); only sad replicates across models. Registry `b1e8.*`; report 30.

## 2026-09-05 13:00Z — Review cycle on the deadline paper, and the two runs it forced
Three cold adversarial reviews (R1 stats 5/10, R2 machinery 4/10, R3 construct 5/10), a mentor review (6/10), a citation audit and a statistical-consistency audit on the first manuscript. Two computations answered them from existing files and one new CPU run: (1) `src/rev3/paper_revision_stats.py` → `paper_revision_stats.json`: every arm against baseline (scenario-blocked, dose-paired) with bootstrap p and BH-FDR, per-dose scenario bands, paired dose-step changes, other-speaker probe slopes, dose-0 readout shifts, medians with calm — registry `rev.*` (5); (2) `src/rev3/a2_lowd_control.py` on box 6 → `a2_lowd_*-v2.json`: projected onto 100 principal components (n ≫ d′) the logistic fits stop separating (training accuracy 0.97 at n = 2000), logistic reproducibility rises 0.39 → 0.90 (standardised) against 0.33 → 0.94 for difference-of-means, and the same-half cosine between estimators stays 0.91–0.94; at full width logistic decodes held-out items better (0.94 vs 0.88) and the two estimators' held-out projections correlate 0.90–0.96 — registry `a2l.*` (3). Consequences: the random direction and random 5-frame are inert by computed contrast; the permuted rank-5 frame's blocks clear BH-FDR against baseline (q 0.001/0.035/0.013) while sad's rank-1 block against baseline does not (q 0.115); the other-speaker projection rises with dose as much as the present one (sad +93 vs +98), so the "receiver's own state" construct is withdrawn; happy saturates and calm reverses. Recalibrated at the IAB @ NeurIPS 2026 workshop bar after revision: R1 7, R2 7, R3 8, mentor 8 (`docs/writeups/reviews/SCORES.md`).

## 2026-09-06 — Report 31: footprint-matched random frame (B1d addendum 2, camera-ready condition 4)
- Run: `b1d_subspace.py --tag qwen36-27b-randk23 --arms none,randsub_all --randsub-k 23 --reps 3` on Qwen3.6-27B, box A, 07:46–08:44Z; same pool (sha 70129f86), same seeds; result `results/rev3/b1d_subspace_qwen36-27b-randk23.json` (prereg sha c3bbe034 found=true); summary `results/rev3/b1d_randk23_summary.json` via `src/rev3/b1d_randk23_summary.py`.
- Footprint: 6.67–6.87 removed per position vs the permuted frame's 6.49–6.89 and the rank-5 random frame's 3.10–3.24 (matched, verified after the run). Readable-affect share removed ≤ 0.13; dose-0 readout shifts ≤ 0.006.
- randsub_all − none (scenario-blocked, dose-paired; blocked fraction; BH-FDR over the five testable emotions): desperate +0.06 (q 0.71), afraid −0.03 (0.71), happy +0.03 (1.00), sad +0.07 (0.71), angry +0.00 (1.00). Calm (untestable) −30.3 [−55.4, −4.3], p 0.024, outside the family.
- Fixed reading (addendum 2): **fitting** — footprint does not make B1d's permuted-label frame bite. Determinism check: none-arm slopes identical to the B1d run's for all six emotions.
- Status: quotable (`b1d23.inert`, `b1d23.footprint_matched`); descriptive control, no decision verdict (file verdict 'indeterminate' because the decision arms were not run). Critique: the frame is Gaussian-random in the full space; a random frame drawn inside the pool's top principal subspace would be the next control, not run.

## 2026-09-06 — Report 32: direction-resampling draws (B1e addendum 3, camera-ready condition 3)
- Runs: `b1e_footprint.py --tag qwen36-27b-draw<s> --split-seed <s> --emotions afraid,sad --arms none,emo_all,permdir_all --reps 3`, s = 1..10, Qwen3.6-27B, boxes B (1-3), C (5-7), D (8-10), A (4), 07:39–09:15Z; same pool (sha 70129f86, symlinked under each tag); prereg sha 4cb36929 found=true in all ten. Files `results/rev3/b1e_footprint_qwen36-27b-draw{1..10}.json`; aggregate `results/rev3/b1e_draws_summary.json` and `docs/writeups/table_b1e_draws.tex` via `src/rev3/b1e_draws_aggregate.py` (no recomputation; driver summaries only).
- Sad: emo_all − permdir_all blocked fraction median 0.35 (0.24–0.56), significant 10/10 (original 0.29); emo_all − none 0.35 (0.23–0.44), 10/10; permdir_all − none 0/10; MC-steer 10/10. **Stable.**
- Afraid: emo_all − permdir_all 0.18 (0.07–0.23), 9/10 (original 0.25 = top of spread); emo_all − none 0.19 (0.08–0.23), 10/10; permdir_all − none 1/10 (draw 6, +0.065); MC-steer passes 4/10 (seeds 1, 4, 9, 10; failing slopes −0.02..+0.09, CIs include zero, two by < 0.01). Per-draw verdict H3 ×4, instrument_failed ×6, all six via afraid's gate. **Block present, gate split-sensitive.**
- Footprint ratio permdir/emo 0.78–0.88 across draws; permdir dose-0 shifts ≤ 0.003.
- Status: quotable (`b1e10.sad_stable`, `b1e10.afraid_gate_fragile`); descriptive, no ensemble verdict. Paper: abstract, intro, §5, Limitations, Appendix B, new Table (appendix) updated; the affect-specific claim narrowed to sad. Critique: the draws vary the split, not the pool or the generation seeds, so the spread is the direction estimate's, not the full pipeline's; B1c/B1d were not re-drawn.

## 2026-09-06 — Report 33: permutation null for the permuted-label rank-5 frame (B1d addendum 3, camera-ready condition 3)
- Runs: `b1d_subspace.py --tag qwen36-27b-perm<s> --arms perm_all --perm-seed <s> --reps 3`, s = 1..4, Qwen3.6-27B, boxes A–D, launched ~10:11Z, done by 10:50Z; same pool (sha 70129f86, symlinked under each tag), generation seeds unchanged, so each run's `perm_all` is contrasted against the B1d run's stored `none` (bit-identical by construction). Files `results/rev3/b1d_subspace_qwen36-27b-perm{1..4}.json`; aggregate `results/rev3/b1d_perm_null.json` and `docs/writeups/table_perm_null.tex` via `src/rev3/b1d_perm_null.py` (scenario-blocked, dose-paired bootstrap, 5000 draws, seed 0; unadjusted).
- Fresh realisations blocking afraid, sad and angry together (CI < 0): 1/4. Over the five realisations (original included): afraid median +0.02 [+0.00, +0.23] sig 2/5; sad +0.34 [+0.12, +0.40] sig 4/5; angry +0.20 [+0.16, +0.23] sig 5/5; desperate sig 0/5, happy sig 0/5; calm untestable. Dose-0 readout shifts ≤ 0.300.
- Fixed reading (addendum 3): **unlucky draw** — three-emotion pattern downgraded to a single-realisation observation; angry 5/5, sad 4/5, afraid 2/5 stated. No decision verdict attached.
- Status: quotable (`b1dperm.reading`, `b1dperm.counts`). Paper: Appendix C Table 7 caption, abstract, introduction, §5 B1d, §6, Limitations, Appendix A and B updated. Critique: four permutations is a small null; the permutations vary the labels only, not the pool or the generation seeds; the contrasts are unadjusted, as registered.
