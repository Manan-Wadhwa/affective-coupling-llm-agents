...
Every claim in `README.md`, `paper.html`, `dossier.html`, `NOVELTY_REVIEW.md`, the
roadmap/spec, and the commit messages, checked against `results/*.json`,
`results/sandbox_pull/*.json` and the producing code. Data was read before prose to
...
fail at the selected β.** `paper.html` §7 / `README.md`: *"at β=0.3 all six emotions
...
6/6 positive. No CIs anywhere in E5. Worse by construction: `coupling_e5_actpass.py`
...
`cmi_passed.py` pools 6 emotions × 29 episodes into one matrix and permutes *across*
...
`paper.html` §5 and `coupling_e2.py`'s docstring call this "the load-bearing result".
...
α=2.0 artifact.** `dual_estimator_battery.py` swept α ∈ {0, 0.5, 1, 2};
`coupling_e2_ci.py` caps α at 1.0 and says so — *"drop the model-breaking alpha=2
...
`paper.html` §A and `NOVELTY_REVIEW.md`.** The underlying estimator-instability finding is
...
`behavioral_induction.py` builds `scramble` by permuting the *desperate* vectors, so
`cheat(desperate) − cheat(scramble)` is ≈0 by construction — as all three files report.
`manip_sig` is false in all three models. `SHARED_WORKSPACE_SPEC.md` itself says a failed
...
`jlens_coupling.py` reads a file not in the repo.
...
`results/sandbox_pull/` re-measures E5, CMI or J-lens. **Corrected.**
...
**18 ·** `coupling_e2_ci.py`'s bootstrap resamples each α cell independently despite the
...
the estimator-instability findings in `NOVELTY_REVIEW.md` §4.1/§4.2/§4.4.
